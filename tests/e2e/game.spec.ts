...
test('mobile controls keep full-size selects and wide maps use 48px scrollable tiles', async ({ page }) => {
	...
	await expect(page.getByRole('button', { name: 'Confirm move', exact: true })).not.toBeVisible()
	const boardBounds = await page.locator('.board').boundingBox()
	const minimapBounds = await page.locator('.overview').boundingBox()
	expect(minimapBounds!.y + minimapBounds!.height).toBeLessThan(boardBounds!.y)
	expect(minimapBounds!.height).toBeLessThanOrEqual(152)
	await page.locator('[data-cell="1"]').click()
	await expect(page.getByRole('button', { name: 'Confirm move', exact: true })).toBeVisible()
	await page.getByRole('button', { name: 'Cancel move', exact: true }).click()
	await expect(page.getByRole('button', { name: 'Confirm move', exact: true })).not.toBeVisible()
	await page.evaluate(() => scrollTo(0, document.documentElement.scrollHeight))
	const barBounds = await page.getByRole('navigation', { name: 'Game controls' }).boundingBox()
	const statsBounds = await page.getByRole('complementary').boundingBox()
	expect(barBounds!.y + barBounds!.height).toBeCloseTo(800, 0)
	expect(statsBounds!.y + statsBounds!.height).toBeLessThan(barBounds!.y)
	await page.getByRole('button', { name: 'Options and help', exact: true }).click()
...
test('capture city and factory, earn income and purchase through the dialog', async ({ page }) => {
	...
	await cell(8).click()
	await expect(page.getByTestId('preview-range')).toHaveText('2–3')
	expect(errors).toEqual([])
})
...
test('combat updates rune-driven health, locks controls and clears a dead attacker', async ({ page }) => {
	...
	await cell(42).click()
	await expect(cell(42).locator('[data-unit]')).toHaveAttribute('data-health', '163')
	await expect(cell(34).locator('[data-unit]')).toHaveCount(0)
	await expect(end).toBeEnabled()
	await expect(page.getByRole('button', { name: 'Confirm move', exact: true, includeHidden: true })).toBeDisabled()
